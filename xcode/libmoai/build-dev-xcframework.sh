...
usage() {
	echo >&2 "usage: $0 [-v] [-d <dir>] [-c Debug|Release] [-D <derived-data-dir>]"
...
build_scheme() {
	local scheme=$1
	local build_dir="${basedir}/intermediates/${scheme}"
	mkdir -p "$build_dir"

	local msg="Building ${scheme}..."
	if $verbose; then
		echo "$msg"
...
combined_dir="${basedir}/combined"
mkdir -p "$combined_dir"
libs=""
for scheme in $osx_schemes; do
	libs="$libs ${basedir}/intermediates/${scheme}/${scheme}.a"
done
...
xcodebuild -create-xcframework \
	-library "${combined_dir}/libmoai-combined.a" \
	-headers "$headers_dir" \
	-output "${basedir}/MoaiSDK-Release.xcframework" \
